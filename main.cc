...
int main(int argc,char * argv[]){
    ...
    //valor_ini = 0.1;
    Grid<> gridEndothelial(250,150,valor_ini,false);//Este es para calcular la derivada y ver hacia donde va la migracion
    valor_ini = 0.0 ;
    ...
    //createRectangle(accessor_2_endothelial,esquina_izquierda,tamanio_tumor,1);
    createRectangle(accessor_1_Tip,esquina_izquierda,1,1);
    createRectangle(accessor_2_Tip,esquina_izquierda,1,1);

    createRectangle(accessor_1_Endothelial_discrete,esquina_izquierda,1,1);
    createRectangle(accessor_2_Endothelial_discrete,esquina_izquierda,1,1);
    ...
    // var->CoordToOffset
    uint64_t nodeCount = gridEndothelialDiscrete.getPtrNano1(typePointer::CPU)->tree().nodeCount(0);
    std::cout<<"NodeCount "<<nodeCount<<std::endl;
    //generateEndothelial(gridEndothelial.getPtrNano1(typePointer::DEVICE),nodeCount,-39,-130,0.5);
    //generateEndothelial(gridEndothelial.getPtrNano2(typePointer::DEVICE),nodeCount,-39,-130,0.5);
